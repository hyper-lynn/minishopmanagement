...
//declear varibble
string usrname, passwd1, passwd2, admin_acc, requsrname, requsrpasswd, choice, confirmation,passwd,phone,add,mytext,press = "Press any key to Continue..........";
string _category,_productname,_procom;
...
{     
      ...
      int tmpqty=0;
      cout<<"Want to change the quantity of product ? (Yes[ y or Y ] or NO [n or N]) : ";
      flag=0;
      while(1)
      {
            cin>>yes4;
      ...
      float tmpdis=0;
      cout<<"Want to change the discount of product ? (Yes[ y or Y ] or NO [n or N]) : ";
      flag=0;
      while(1)
      {
            cin>>yes5;
...
void place_order()
{
      ...
      // ofstream history(value+"_bill_record",ios ::app);
      ...
      while(inFile.read((char *) &cust, sizeof(customer)))
      {
            if(cust.getcustid()==value)
            {
                  ofstream history(usrname+"_bill_record",ios ::app);
                  history << "Date and Time : "<< currentDateTime()  << endl;
                  history<< "Customer's id : " <<value << endl;
                  history<< "Customer's name : " << usrname << endl;
                  history<< "Customer's address : " << add << endl;
                  history << "Customer's phonenumber : " << phone << endl;
      ...
      if(value==1)
      {
            ...
            do
            {
                  ...
                  cin>>pr1;
                  ofstream history(usrname+"_bill_record",ios ::app);
                  history << "Product ID : "<< pr1 << endl;
                  history << "Product's Catergory : "<< _category << endl;
                  history << "Product's Name : "<<_productname << endl;
                  history << "Product's Company :  " << _procom << endl;
                  history.close();
            ...
            {     
             ...
             cin>>vb;
            ofstream history(usrname+"_bill_record",ios ::app);
            history << "Total Ammount   : " << total << endl;
            history << "CGST            : " << ttaxt/2<<"%"<<endl;
            history << "SGST            : " << ttaxt/2<<"%"<<endl;
            history << "NET TOTAL       : " <<"Kyats."<<fixed<<round(total+((ttaxt*total)/100))<<endl<<endl;
            history <<"ROUND OFF AMOUNT : "<<"Kyats."<<setprecision(2)<<(round(total+((ttaxt*total)/100))-(total+((ttaxt*total)/100)))<<endl;
            history << "NET AMOUNT DUE  : "<<"Kyats."<<fixed<<round(total+((ttaxt*total)/100))<<endl<<endl;
            history << "-------------------------------------------------------------------------" << endl;
            history << "Enter CASH Value: " << "Kyats." << vb << endl;
            history.close();
...
void delete_acc()
{
      ...
      cin >> choice;
      cout << "Note : If you created the acc and try to delete it in same process, You can't do it!" <<endl;
      // cout << "Note : if you create the admin acc in this process and try to delete it,You can not delete the acc. So restart the program !" << endl;
      ...
      {     
            ...
            tmp.close();
            fin.close();
            rename("admin","old_admin");
            rename("tmp1","admin");
            cout << "Successfully Deleteed < Admin \"" << usrname << "\" account >" << endl;
